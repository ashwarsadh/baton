...
const LEDGER_CMD_TIMEOUT_MS = 6000;
const LEDGER_TTL_MS = 20000;
let _ledger = { at: 0, byId: null };

function collectedLedger(now = Date.now()) {
  if (_ledger.byId && (now - _ledger.at) < LEDGER_TTL_MS) return _ledger.byId;
  let byId = null;
  try {
    const dir = path.join(require('./config').CLAUDE_HOME, 'skills', 'conductor', 'scripts');
    const ledgerFile = path.join(require('./config').CLAUDE_HOME, 'conductor', 'collected.jsonl');
    if (fs.existsSync(dir) && fs.existsSync(ledgerFile)) {
      const r = require('child_process').spawnSync('python', ['build_index.py', 'goal', 'collected'],
        { cwd: dir, timeout: LEDGER_CMD_TIMEOUT_MS, encoding: 'utf8', windowsHide: true });
      if (!r.error && r.status === 0 && r.stdout) {
        const m = {};
        for (const line of String(r.stdout).split(/\r?\n/)) {
          const hit = /^(local_[0-9a-f]+)\s+(banked|moved-since|cannot-verify)\b/.exec(line.trim());
          if (hit) m[hit[1]] = hit[2];
        }
        if (Object.keys(m).length) byId = m;
      }
    }
  } catch { byId = null; }
  _ledger = { at: now, byId };
...
function alreadyBanked(sessionId, now) {
  const led = collectedLedger(now);
  if (!led) return false;
  const key = Object.keys(led).find(k => String(sessionId || '').startsWith(k));
  if (!key) return false;
  return led[key] === 'banked';
}
...
function scanSessions(state, masters, cfg, sessions, firstRun, now = Date.now()) {
  ...
  for (const s of sessions || []) {
    ...
    if (s.state === 'unread' && conductorId && master.sessionId === conductorId
        && alreadyBanked(s.sessionId, now)) {
      continue;
    }
...
module.exports = {
  ...
  flush, tick, pushEvent, _setSender, _setTaskSource, blank,
};
