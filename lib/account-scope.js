...
const STORE = path.join(HOME, 'AppData', 'Roaming', 'Claude', 'claude-code-sessions');
...
const DESKTOP_CONFIG = path.join(HOME, 'AppData', 'Roaming', 'Claude', 'config.json');
const LABELS = path.join(HOME, '.claude', 'migrate', 'account-labels.json');

function labelFor(accountId) {
  try { return (JSON.parse(fs.readFileSync(LABELS, 'utf8')) || {})[accountId] || null; } catch { return null; }
}
...
function activeScope() {
  ...
  const base = { ok: true, state: 'known', accountId, account: labelFor(accountId), scopes: summary,
...
function raise(change, now = Date.now()) {
  ...
  const q = {
    ...
    fromAccount: labelFor(String(change.from).split('/')[0]),
    toAccount: labelFor(String(change.to).split('/')[0]),
...
module.exports = { scopesOnDisk, activeScope, note, readState, labelFor,
                   raise, pending, answer, readPending,
