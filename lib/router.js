...
const MODEL = require('./config').get().workers.defaultModel;
const LADDER = [
  { model: MODEL, effort: 'low',    lean: false, tier: 0, label: 'opus-5.5/low' },
  { model: MODEL, effort: 'medium', lean: false, tier: 1, label: 'opus-5.5/medium' },
  { model: MODEL, effort: 'high',   lean: false, tier: 2, label: 'opus-5.5/high' },
  { model: MODEL, effort: 'max',    lean: false, tier: 3, label: 'opus-5.5/max' },
];
const MID_TIER = 1;
const TOP_ROUTED_TIER = LADDER.length - 2;
const tierOf = (model, effort) => {
  const hit = LADDER.find(l => l.model === model && l.effort === effort);
  if (hit) return hit.tier;
  const byEffort = LADDER.find(l => l.effort === (effort === 'xhigh' ? 'high' : effort));
  return byEffort ? byEffort.tier : MID_TIER;
};
const rung = tier => LADDER[Math.max(0, Math.min(LADDER.length - 1, tier))];
...
function escalate(model, effort) {
  ...
  if (t >= LADDER.length - 1) return null;
  const n = rung(t + 1);
...
module.exports = { route, classifyRules, escalate, needsModelAssist, LADDER, MODEL, tierOf, rung };
