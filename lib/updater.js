...
// Only then, and only when no Baton task is running, does the daemon hand over to a detached script
...
function apply(file, version, { dry = false } = {}) {
  ...
  // Created through WMI, so the script is not a child of this daemon: a daemon started by a scheduled
  // task (or a terminal) lives in a job object that can take its children down when it ends, and the
  // installer ends it. run-hidden.vbs keeps the console window away.
  const vbs = path.join(ROOT, 'scripts', 'run-hidden.vbs');
  const line = `wscript.exe "${vbs}" cmd.exe /d /c "${cmd}"`.replace(/'/g, "''");
  const r = require('child_process').spawnSync('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command',
    `$r = Invoke-CimMethod -ClassName Win32_Process -MethodName Create -Arguments @{ CommandLine = '${line}'; CurrentDirectory = '${DIR.replace(/'/g, "''")}' }; "$($r.ReturnValue) $($r.ProcessId)"`],
  { encoding: 'utf8', windowsHide: true, timeout: 60000 });
  const [rc, pid] = String(r.stdout || '').trim().split(/\s+/);
  if (rc === '0') return { ok: true, script: cmd, pid: Number(pid), via: 'wmi' };
  log('WMI launch failed (' + String(r.stdout || r.stderr || r.error).trim().slice(0, 200) + '); falling back to a detached child');
  const child = spawn(process.env.ComSpec || 'cmd.exe', ['/d', '/c', cmd], { detached: true, stdio: 'ignore', windowsHide: true, cwd: DIR });
  child.unref();
  return { ok: true, script: cmd, pid: child.pid, via: 'detached' };
}
...
async function tick({ force = false, applyNow = true } = {}) {
  const s = settings();
  const st = state();
  // After a restart into the version we installed, record that it landed.
  if (st.installing && cmpVersion(current(), st.installing.to) >= 0) {
  ...
  return { ...st.check, action: 'installing', script: r.script };
}
...
module.exports = { DEFAULTS, PUBLIC_KEY, cmpVersion, installKind, assetName, verifySums, listedHash, check, fetchVerified, busyReason, apply, tick, cli, state };
