...
const DEBUGGER_REASONS = {
  ...
  7: 'Windows would not let Baton bring Claude Desktop to the front (focus-stealing protection). Click on the Claude window once, then try again.',
  8: 'Blocked by Windows: the computer\'s session is disconnected (Remote Desktop is closed), and Windows does not deliver clicks to a disconnected session. Connect to the computer, then try again; Baton also does it by itself when you reconnect.',
  9: 'Blocked by Windows: the computer is locked, and Windows does not deliver clicks behind the lock screen. Unlock it, then try again; Baton also does it by itself once it is unlocked.',
  10: 'Claude Desktop is not signed in yet. Sign in; Baton then turns the debugger on by itself.',
  11: 'Dry run finished: every step reached the menu item; nothing was clicked.',
  12: 'You kept using the computer during the countdown, so Baton held off. It tries again in a minute.',
};
/** Run the macro once. Always resolves to { ok, code, message, detail } — never a silent no-op. */
function enableDebugger({ countdown = 3 } = {}) {
  const macro = process.env.BATON_DEBUGGER_MACRO || path.join(SCRIPTS, 'enable-debugger.ps1');   // env: test seam
  ...
  if (!fs.existsSync(macro)) return Promise.resolve({ ok: false, code: -1, message: 'Baton\'s enable-debugger.ps1 is missing from its install folder. Reinstall Baton.' });
  return new Promise(resolve => {
    execFile('powershell', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', macro, '-Port', String(cdpPort()), '-Countdown', String(countdown)],
...
async function repairCdp(log) {
  ...
  log.push('the port is open but Baton cannot reach the debugger yet');
  return false;
}
